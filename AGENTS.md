...
The repo is public, and the owner's real job search runs through this code. `.gitignore` and two guard tests (`tests/test_no_third_party_pii.py`, `tests/test_no_personal_files.py`) are lines of defence; you are the last one, and this applies to every agent and model that ever works here.
...
7. **Personal planning, briefs, notes, exports and identifiers never enter the tracked tree, and every agent checks before every commit.** Duc's rule after it happened more than once (a personal project brief committed under `docs/plans/` and moved out by hand; an Apple developer team id written into the tracked Xcode project by a signed build). Where they go: `data/private_docs/` for documents and notes, `data/` for everything else, both gitignored; a tracked file references the path. Before every commit and before opening a pull request, run the `private-check` skill: `git status --porcelain`, then `pytest tests/test_no_third_party_pii.py tests/test_no_personal_files.py`. The second guard fails on any `docs/` file whose name says brief, handoff, personal or private, on any personal document extension, on a non-empty `DEVELOPMENT_TEAM` in the tracked Xcode project, and on a personal email address. A new kind of personal file gets a `.gitignore` pattern and a line in that guard in the same commit that removes it. Something already committed by mistake is removed in the next commit, named in the message, and reported to Duc, because the public history is his to rewrite or not.
...
- **Boundaries set by decision, not backlog** (do not extend without asking first): autofill fills and never submits; nothing automates LinkedIn; the Claude Code handoff drafts and never spawns; Workday is never autofilled; Chromium stays out of the container image; nothing uploads to YouTube, and an arbitrary YouTube source is embed-only, never downloaded (learning reels, 2026-09-13; `tests/test_reels_boundaries.py` catches the obvious violation, review catches the rest).
...
**What the two share, and what they cannot.** The instruction file is shared (`AGENTS.md`, imported by `CLAUDE.md`). The **skills** are shared by a symlink: `.agents/skills` points at `../.claude/skills`, so there is one copy of the nine vendored `SKILL.md` files and the attribution beside them. It is relative, so moving the repo does not break it. **Hooks are not configured.** A future hook may share one repository script if both harnesses' documented protocols support it, but each harness would still need its own registration and real-run verification. Do not describe a planned hook as active. **Plugins, personal memory and harness config are not shareable and should not be faked**: they are different systems with different marketplaces and formats (`~/.claude/settings.json` against `~/.codex/config.toml`). Anything that must reach both belongs in `AGENTS.md`, a skill, a tracked script, or `docs/log/`.
