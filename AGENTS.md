...
**What the two share, and what they cannot.** The instruction file is shared (`AGENTS.md`, imported by `CLAUDE.md`). The **skills** are shared by a symlink: `.agents/skills` points at `../.claude/skills`, so there is one copy of the nine vendored `SKILL.md` files and the attribution beside them. It is relative, so moving the repo does not break it. **Hooks** are shared by script rather than by config: both harnesses use the same event names (`PreToolUse`, `PostToolUse`, `SessionStart`, `Stop`) and the same wire format, a JSON event on stdin and a `hookSpecificOutput.permissionDecision` reply, so one script in `scripts/` is registered twice, once in `.claude/settings.json` and once in Codex's hooks file. **Plugins and harness config are not shareable and should not be faked**: they are different systems with different marketplaces and formats (`~/.claude/settings.json` against `~/.codex/config.toml`). Anything that must reach both belongs in `AGENTS.md`, a skill, or a script.
