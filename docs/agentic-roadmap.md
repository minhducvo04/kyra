...
- **Whole-system audit — done, 2026-09-04 evening, see `docs/industry-standards.md`.** Test suite (56 tests), lint + CI, centralized `DATA_DIR`, logging, the one-page loop rebuilt on measured overflow, `resume_guard.py` fact check, fit status surfaced in the UI. Open follow-ups with recommendations: `docs/needs-your-input.md`. Long-range shape: `docs/v2-outline.md`.
- **Scheduler (daily digest of due reminders / due reviews / news via launchd)** — concrete and small (~1 h); needs Duc's call on timing and delivery (notification vs. spoken greeting). See `docs/needs-your-input.md` #5.
