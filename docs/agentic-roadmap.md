...
- ~~Router classifier fine-tune~~ — **done 2026-09-05, see `docs/router-finetune.md`**: LoRA on Qwen2.5-1.5B replaces the few-shot 3B in production (73.8% → 88.5% on a handwritten held-out set, 4x faster, 17x fewer prompt tokens). Next: second seed for error bars, fold real `router.log` turns into training once there are enough.

- ~~Resume "Detailed" mode (pick-and-choose with fit scoring)~~ — **built and verified, 2026-09-04, see CLAUDE.md.** Two endpoints (`/api/job/resume-fit/analyze`, `/api/job/resume-fit/generate`), a section-then-entry-grouped checklist UI with "N% fit — Priority" badges, hierarchical entry/bullet cut decisions, and overflow reported as sorted cut-suggestions rather than auto-trimmed (Duc's explicit answer to the open question this entry used to pose). Real bugs found and fixed in the process: bullets not sharing a stable `entry` key with their parent, a misclassified non-bullet intro sentence causing a bad `\item` insertion on generate, and an empty section heading left behind when every entry in it was cut.
...
- **Whole-system audit — done, 2026-09-04 evening, see `docs/industry-standards.md`.** Test suite (56 tests), lint + CI, centralized `DATA_DIR`, logging, the one-page loop rebuilt on measured overflow, `resume_guard.py` fact check, fit status surfaced in the UI. Open follow-ups with recommendations: `data/private_docs/needs-your-input.md`. Long-range shape: `docs/v2-outline.md`.
- **Scheduler (daily digest of due reminders / due reviews / news via launchd)** — concrete and small (~1 h); needs Duc's call on timing and delivery (notification vs. spoken greeting). See `data/private_docs/needs-your-input.md` #5.
...
- **Usage-pattern observation — v1 built, 2026-09-03** — Duc's idea: notice repeated behavior (e.g. asking Kyra to draft a resume several times before Job Application Auto existed) and surface a suggestion to build a real pipeline for it, rather than Duc having to notice that himself.
  - `src/companion/patterns.py` (`find_patterns`, `write_observation`) + `scripts/analyze_patterns.py` (CLI, same shape as `scripts/summarize_book.py`): counts `path == "tool"` reasons in `data/router.log` over a rolling window (default 7 days), flags anything crossing a threshold (default 3x). text-path reasons are skipped entirely - "casual chat"/"explaining a topic" dominate that path and aren't a useful signal, while tool-path reasons are already specific.
  - On a flagged hit, **one** Claude call turns the raw count into a plain-language observation - not called per-candidate, per-turn, or on a schedule this module controls itself. Saved to `data/pattern_observations.md` (dated, human-readable, git-diffable) and to `memory_notes` under an `observations` category, so Kyra can bring it up naturally in a later conversation rather than this being a push notification.
  - Run manually for now (`python3 scripts/analyze_patterns.py [--days N] [--threshold N]`); a `launchd`/cron schedule is a natural follow-up, not built yet.
  - **Real cleanup this surfaced**: `data/router.log` was found to be 100% test pollution from same-day classifier re-testing (193 entries, all within a 2-hour window, frequency distribution exactly matching the test suites run that day) - backed up to `/tmp/router.log.pre-clean.bak` and cleared before building this, since running pattern analysis on test noise would have produced a garbage first observation. A gap in the project's own "clean up test pollution" discipline (classifier re-test scripts call `router.route()` directly, which logs same as production) - worth remembering to clean `router.log` too after future classifier re-tests, not just the DB stores.
  - Verified with a real Claude call against synthetic data (5x-repeated reason → a specific, genuinely useful observation, not generic advice) and against the real (now-clean) log's correct "nothing to report" path, at no cost, since the threshold check short-circuits before any API call.
  - **v2, closer to what Duc originally described** ("note down what it notices"): instead of pure counting, periodically hand Claude a batch of recent router.log entries + conversation snippets and ask it to reason about repeated workflows/friction, not just tally frequency - catches things counting can't, but costs a real call every run and needs prompt-tuning against noisy suggestions. Not built - natural upgrade once v1 proves useful in practice.
  - **On "another agent to observe from afar"**: a Claude Code subagent watching Kyra's data files would only run while a Claude Code session is open - not a fit, since Kyra runs standalone. The "agent" here is Kyra's own script against her own logs, which is what got built.
