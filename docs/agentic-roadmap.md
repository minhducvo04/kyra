...
- **Usage-pattern observation** (proposed 2026-09-03, not yet built) — Duc's idea: notice repeated behavior (e.g. asking Kyra to draft a resume several times before Job Application Auto existed) and surface a suggestion to build a real pipeline for it, rather than Duc having to notice that himself. Design sketch, not yet started:
  - **What already exists to mine**: `data/router.log` (JSONL, every routing decision + reason - repeated tool calls or repeated text-path reasons are the clearest signal), `ChromaMemoryStore` (full conversation history), `data/memory_notes/` (curated facts).
  - **Recommended v1** (cheap, matches the project's existing "start lean" pattern - RSS not scraping, draft-only handoff): a standalone script (same shape as `scripts/summarize_book.py`), run manually or via `launchd`/cron, that counts tool/topic frequency in `router.log` over a rolling window and flags anything crossing a threshold (e.g. the same tool 3+ times in 7 days). On a hit, one cheap Claude call turns the raw counts into a plain-language observation, saved to `data/pattern_observations.md` (human-readable, git-diffable) and/or a `memory_notes` "observations" category so Kyra can mention it naturally next conversation - not a push notification, just something she knows to bring up.
  - **v2, more like what Duc described** ("note down what it notices"): instead of pure counting, periodically hand Claude a batch of recent router.log entries + conversation snippets and ask it to reason about repeated workflows/friction, not just tally frequency - catches things counting can't (e.g. "Duc keeps re-explaining the same project context"), but costs a real Claude call every run and needs prompt-tuning to avoid noisy, low-value suggestions. Natural upgrade once v1 proves the idea is useful.
  - **On "another agent to observe from afar"**: worth being precise about which kind. A Claude Code subagent watching Kyra's data files would only run while a Claude Code session is open - not a fit, since Kyra is meant to run standalone (`chat.py`/`voice_chat.py`/`web_ui.py`, no dependency on this coding environment). The "agent" here should be Kyra's own capability - a script she (or a scheduler) runs against her own logs - not a Claude Code construct.
  - **Not started because it's genuinely a new subsystem with an ongoing cost** (every analysis run that calls Claude costs money, however small) - flagged here with a recommended approach rather than built speculatively. Say the word and v1 is a small, scoped build.
