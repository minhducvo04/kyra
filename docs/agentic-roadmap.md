...
- **Resume "Detailed" mode (pick-and-choose with fit scoring)** — Duc's ask, 2026-09-04: for mass-applying, he wants a fast automatic mode (built - `latex_resume`/"Resume — Fast" in the DRAFT tab, `optimize_latex_resume_one_page()`) *and* a slower, reviewable mode where he sees each candidate piece of content (a bullet, a project, a course) tagged with a relevance score/priority before anything gets cut, and picks what actually goes in rather than trusting Kyra's automatic judgment. Design sketch, not yet built: (1) an "analyze" call reads the resume + job context and returns each candidate content block as structured data (section, short label, a 0-100 fit score, a High/Medium/Low priority bucket, a one-line reason) without editing the LaTeX yet; (2) the UI renders that as a checklist grouped by section, pre-checked per the recommendation, with the score/priority shown as a badge; (3) Duc adjusts checkboxes and hits Generate, which edits the LaTeX to reflect exactly his selection (same no-invention rule as Fast mode) and compiles/reports the real page count - if it doesn't fit, tell him which more to cut rather than silently overriding his picks, since the whole point of this mode is he's in control of what gets cut, not Kyra. Open questions before building: how the fit score should actually be computed/calibrated (it's an LLM judgment call, not a real metric, so the number needs to be framed honestly), and whether "doesn't fit after your picks" should offer one more automatic-trim fallback or just tell him to deselect more.
