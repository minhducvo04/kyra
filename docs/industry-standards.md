...
| | Before | After | Why |
|---|---|---|---|
| ✅ **Advisory tool boundary (2026-09-09)** | A model could read proposed steps in a tool result and request an action in the next round. | A `suggest_initiatives` response runs once and ends the tool loop; sibling calls are suppressed and evidence is rendered directly. Regression tests cover actions before/after the suggestion in the same batch, duplicate requests, failures, and follow-up rounds. | A proposal must remain data until the user chooses an action. Reference validation and a verb filter do not establish an execution boundary. |
...
| ✅ **Storage behind an interface + migrations (2026-09-06, v2 slice 2)** | Three stores each hand-wrote `CREATE TABLE` SQL against sqlite3; no migration tool; no way to run on a server database. | SQLAlchemy Core tables (`schema.py`) matching the v1 DDL so existing data reads unchanged; `DATABASE_URL` switches every store to Postgres; Alembic with an autogenerated initial revision; store tests parametrized over both backends with Postgres as a CI service. | Schema lives in one declared place, changes are versioned migrations, and the same code runs on the laptop's SQLite and the server's Postgres - that's the property that makes a deploy boring. |
| ✅ **Long work off the request path (2026-09-06, v2 slice 3)** | A 1-2 minute resume loop ran inside one HTTP request; the UI showed a frozen button; a dropped connection lost the work. | `JobQueue` interface with a DB-backed implementation (`jobs` table, `SKIP LOCKED` on Postgres), a worker loop (thread on the laptop, separate container service in compose), and SSE progress the DRAFT tab streams live. | Request/response is for milliseconds; anything longer is a job with durable state, progress, and a retry story - and the queue behind an interface is what lets SQS replace the table without touching the API. |
