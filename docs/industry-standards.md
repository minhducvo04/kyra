...
| 🟡 **Other endpoints** | ~10 endpoints still return `{"error": ...}` with 200. | Left as-is tonight: the front end reads `data.error` consistently, and changing both sides at once was out of scope for a night that had to stay verifiable. | Documented as v2 item: consistent error envelope + correct codes, `HTTPException` everywhere. |
